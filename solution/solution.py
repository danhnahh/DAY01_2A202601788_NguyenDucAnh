...
def chat_with_system_prompt(
    ...
    from openai import OpenAI          # ← import TRONG hàm (bắt buộc!)
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"),
                    base_url=os.getenv("OPENAI_BASE_URL"))  # đọc base_url từ .env
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        return max(1, len(text) // 4)   # fallback: ~1 token/4 ký tự
...
def streaming_chatbot() -> None:
    ...
    client = OpenAI(
        ...
        base_url=os.getenv("OPENAI_BASE_URL"),  # Gemini endpoint từ .env
    )
    history = []
    while True:
        user_msg = input("Bạn: ")
        ...
        stream = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=messages,
            stream=True,          # ← bắt buộc, test kiểm tra điều này
        )
        print("Trợ lý: ", end="", flush=True)
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""   # ← or "" cho chunk None cuối
            print(delta, end="", flush=True)
            reply += delta
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise                              # hết lượt → ném lỗi gốc ra ngoài
...
def run_assistant(
    ...
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    while True:
        # ① check max_turns TRƯỚC get_input — bắt buộc!
        if max_turns is not None and num_turns >= max_turns:
            break
        user_msg = get_input()
        if user_msg.strip().lower() in ("quit", "exit"):
            break
        # ② System prompt đứng đầu, không bị cắt theo history
        messages = (
            [{"role": "system", "content": persona}]
            + history
            + [{"role": "user", "content": user_msg}]
        )
        # ③ Bọc API call trong retry_with_backoff
